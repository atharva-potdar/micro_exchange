...
TEST(Framer, ParseConcatenatedMessages) {
  ...
  WireOrderAdd m1;
  m1.hdr = {sizeof(WireOrderAdd), static_cast<uint8_t>(MessageType::OrderAdd)};
  m1.id = 1;
  m1.price = 100;
  m1.qty = 10;
  m1.side = 1;
  ...
  WireOrderCancel m2;
  m2.hdr = {sizeof(WireOrderCancel),
            static_cast<uint8_t>(MessageType::OrderCancel)};
  m2.id = 2;

  WireOrderAdd m3;
  m3.hdr = {sizeof(WireOrderAdd), static_cast<uint8_t>(MessageType::OrderAdd)};
  m3.id = 3;
  m3.price = 200;
  m3.qty = 20;
  m3.side = 1;
